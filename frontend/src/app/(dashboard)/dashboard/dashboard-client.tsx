    ...
    <motion.div variants={container} initial="hidden" animate="show" className="max-w-[1500px] mx-auto pb-24 px-2">
      ...
      <motion.div variants={item} className="grid grid-cols-1 lg:grid-cols-12 gap-5 mb-5">
        ...
        <div className="lg:col-span-4 min-h-[260px]">
          <HealthScoreCard score={(stats?.totalDocuments === 0 && stats?.activeMedications === 0 && stats?.totalLabResults === 0) ? 0 : healthScore} trend={healthTrend} isLoading={statsLoading} />
        </div>
        ...
        <div className="lg:col-span-5 min-h-[260px] surface-card rounded-[1.5rem] border border-border/50 p-6 flex flex-col justify-between">
          <div>
            ...
            {aiLoading ? (
              ...
              <p className="text-sm text-foreground/80 leading-relaxed font-medium line-clamp-4">
                {stats?.totalDocuments === 0 ? "No medical documents provided yet. Upload your first document to generate your AI Health Summary and track your metrics." : (aiSummary?.summary || "Your overall health is looking good! Your lab results are mostly in the normal range. Continue your current medications and maintain a healthy lifestyle.")}
              </p>
            )}
          </div>
          ...
          <div className="flex items-center gap-2">
            <div className="border border-border/40 rounded-lg px-3 py-1.5 bg-background">
              ...
              <p className={cn("text-xs font-bold", (stats?.totalDocuments === 0 || !stats?.recentLabResults?.length) ? "text-muted-foreground" : "text-emerald-600 dark:text-emerald-400")}>{(stats?.totalDocuments === 0 || !stats?.recentLabResults?.length) ? "No Data" : "Normal"}</p>
            </div>
            <div className="border border-border/40 rounded-lg px-3 py-1.5 bg-background">
              <p className="text-[10px] text-muted-foreground font-semibold">Blood Sugar</p>
              <p className={cn("text-xs font-bold", (stats?.totalDocuments === 0 || !stats?.recentLabResults?.length) ? "text-muted-foreground" : "text-emerald-600 dark:text-emerald-400")}>{(stats?.totalDocuments === 0 || !stats?.recentLabResults?.length) ? "No Data" : "Controlled"}</p>
            </div>
            <div className="border border-border/40 rounded-lg px-3 py-1.5 bg-background">
              <p className="text-[10px] text-muted-foreground font-semibold">Cholesterol</p>
              <p className={cn("text-xs font-bold", (stats?.totalDocuments === 0 || !stats?.recentLabResults?.length) ? "text-muted-foreground" : "text-amber-600 dark:text-amber-400")}>{(stats?.totalDocuments === 0 || !stats?.recentLabResults?.length) ? "No Data" : "Borderline"}</p>
            </div>
          </div>
      ...
      <motion.div variants={item} className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
        <MetricCard
          title="Documents" value={stats?.totalDocuments ?? 0} subtitle={`+${stats?.documentsThisMonth ?? 0} this month`}
          icon={FileText} colorClass={{ bg: "bg-blue-500/10", text: "text-blue-500" }}
          trendData={stats?.totalDocuments ? [1, 2, 2.5, 2, 4, stats.totalDocuments] : []} trendColor="#3b82f6"
        />
        <MetricCard
          title="Active Medications" value={stats?.activeMedications ?? 0} subtitle={`${stats?.upcomingRemindersToday ?? 0} due today`} subtitleColor="text-amber-500"
          icon={Pill} colorClass={{ bg: "bg-emerald-500/10", text: "text-emerald-500" }}
          trendData={stats?.activeMedications ? [5, 5, 4, 5, stats.activeMedications] : []} trendColor="#10b981"
        />
        <MetricCard
          title="Lab Results" value={stats?.totalLabResults ?? 0} subtitle={`+${stats?.totalDocuments ? Math.min(stats.totalLabResults || 0, 3) : 0} this month`}
          icon={FlaskConical} colorClass={{ bg: "bg-purple-500/10", text: "text-purple-500" }}
          trendData={stats?.totalLabResults ? [10, 15, 12, 18, 22, stats.totalLabResults] : []} trendColor="#a855f7"
        />
      </motion.div>
        ...
        <div className="surface-card rounded-[1.5rem] border border-border/50 p-6 flex flex-col">
          ...
          <div className="flex-1 space-y-4">
            {statsLoading ? Array(4).fill(0).map((_,i) => <div key={i} className="skeleton h-10 w-full" />) : 
              (stats?.recentLabResults && stats.recentLabResults.length > 0 ? stats.recentLabResults.slice(0, 4).map((lab: any) => {
                ...
                <div className="flex flex-col items-center justify-center py-8 text-center">
                  ...
                  <p className="text-xs font-bold text-muted-foreground">No lab reports uploaded</p>
                  <p className="text-[10px] text-muted-foreground/70 mt-0.5">Upload lab reports to automatically extract and track results.</p>
                </div>
              ))
            }
          </div>
          ...
          <div className="flex-1 flex items-center gap-6">
            ...
            <div className="relative flex h-24 w-24 shrink-0 items-center justify-center">
              <svg className="absolute inset-0 h-full w-full -rotate-90" viewBox="0 0 100 100">
                ...
                <circle cx="50" cy="50" r="40" fill="none" stroke="#10b981" strokeWidth="8" strokeLinecap="round" strokeDasharray="251.2" strokeDashoffset={251.2 * ((medications?.length || stats?.activeMedications) ? 0.2 : 1)} />
              </svg>
              <div className="relative z-10 flex flex-col items-center">
                <span className="text-xl font-extrabold text-foreground">{(medications?.length || stats?.activeMedications) ? "80%" : "0%"}</span>
              </div>
            ...
            <div className="flex-1 space-y-4 pt-2">
              ...
              {(!medications || medications.length === 0) && (stats?.activeMedications === 0 || stats?.totalDocuments === 0) ? (
                <div className="py-4 text-center sm:text-left text-muted-foreground">
                  <p className="text-xs font-bold">No active medications scheduled</p>
                  <p className="text-[10px] text-muted-foreground/70 mt-0.5">Upload a prescription or add medications to track daily intake.</p>
                </div>
              ) : medications && medications.length > 0 ? (
                medications.slice(0, 3).map((med: any, idx: number) => (
                  <div key={med.id || idx} className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground font-medium">{med.frequency || med.dosage || "As prescribed"}</p>
                    </div>
                  </div>
                ))
              ) : (
                <>
                  <div className="flex items-start gap-3">
                    <div className="mt-0.5"><CheckCircle2 className="h-4 w-4 text-emerald-500" /></div>
                    <div>
                      <p className="text-sm font-bold text-foreground">Active Medication</p>
        ...
        <div className="surface-card rounded-[1.5rem] border border-border/50 p-6 flex flex-col">
          ...
          <div className="flex-1 relative">
            <div className="space-y-6">
              {statsLoading ? Array(3).fill(0).map((_,i) => <div key={i} className="skeleton h-12 w-full mb-3" />) : 
                (timelineEvents && timelineEvents.length > 0 ? (
                  <div className="relative">
                    <div className="absolute left-[15px] top-2 bottom-2 w-0.5 bg-border/60" />
                    <div className="space-y-6">
                      {timelineEvents.slice(0, 3).map((ev: any) => (
                        <div key={ev.id} className="relative flex gap-4">
                          ...
                          <div className="flex-1 pb-2">
                            <div className="flex justify-between items-center mb-1">
                              <div className="flex items-center gap-2">
                                <span className="text-[10px] font-bold bg-muted px-2 py-0.5 rounded-md">{ev.eventType || "Event"}</span>
                                <span className="text-[10px] text-muted-foreground font-medium">{formatDate(ev.eventDate)}</span>
                              </div>
                            </div>
                  ...
                  <div className="flex flex-col items-center justify-center py-8 text-center text-muted-foreground">
                    ...
                    <p className="text-xs font-bold text-muted-foreground">No health timeline events recorded</p>
                    <p className="text-[10px] text-muted-foreground/70 mt-0.5">Upload prescriptions or clinical reports to build your health timeline.</p>
                  </div>
                ))
              }
            </div>
      ...
      <motion.div variants={item} className="surface-card rounded-[1.5rem] border border-border/50 p-6">
         ...
         <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {stats?.totalDocuments === 0 ? (
              <div className="col-span-4 flex items-center justify-center py-6 text-center text-muted-foreground">
                <p className="text-xs font-medium">No medical documents uploaded yet. Upload your records to generate personalized AI health recommendations.</p>
              </div>
            ) : (
