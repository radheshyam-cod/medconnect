/**
 * Live integration test for Alchemyst AI API.
 *
 * Usage:
 *   ALCHEMYST_API_KEY="sk-..." npx ts-node --skip-project backend/test-alchemyst.ts
 *
 ...
 *
 * It does NOT depend on NestJS — it calls Alchemyst directly via fetch.
 */
...
interface AlchemystContextItem {
  content: string;
  score: number;
  createdAt: string;
  updatedAt: string;
...
async function main() {
  ...
  if (!apiKey) {
    ...
    console.error('   Usage: ALCHEMYST_API_KEY="sk-..." npx ts-node --skip-project backend/test-alchemyst.ts');
    process.exit(1);
  }
  ...
  try {
    ...
    if (!searchResponse.ok) {
      ...
      const parsed = JSON.parse(searchBody) as SearchResponse;
      ...
      if (contexts.length > 0) {
        ...
        for (let i = 0; i < Math.min(3, contexts.length); i++) {
          ...
          console.log(`  Content:   ${(c.content || '').slice(0, 200)}${c.content?.length > 200 ? '...' : ''}`);
    ...
    const addResponse = await fetch(`${ALCHEMYST_BASE_URL}${ADD_PATH}`, {
      ...
      body: JSON.stringify({
        ...
        scope: 'internal',
        metadata: {
          testName: 'alchemyst-integration-verification',
          testTimestamp: new Date().toISOString(),
        },
      }),
    });
    ...
    if (!addResponse.ok) {
      ...
      const reSearchBody = await reSearchResponse.json() as SearchResponse;
